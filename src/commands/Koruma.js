...
module.exports = {
  ...
  async execute(interaction, bot) {
    ...
    const embed = new MessageEmbed()
      .setDescription(`${interaction.user.toString()}, \`${moment(Date.now()).format("LLL")}\` tarihinden itibaren \`${interaction.guild.name}\` sunucusundaki yetki verilerinin işlem tablosu aşağıda belirtilmiştir.`)
      .addFields(
        {
          name: "\n\u200b",
    ...
    const collector = interaction.channel.createMessageComponentCollector({ filter, componentType: 'BUTTON', max: 1, time: 20000 });

    collector.on("collect", async interaction => {
      let veri = await SafeMember.findOne({ guildID: interaction.guild.id }) || { "Permissions": [] };

      const izinler = ["ADMINISTRATOR"];
      const data = [];
      ...
      if (interaction.customId === "aç") {
        if (veri.Permissions.length > 0) {
          ...
          interaction.update({ content: `Lütfen mevcut izinleri kaydediniz!`, embeds: [], components: [] });
        }
      }
