...
const Config = require("./src/Config/BotConfig.json");
...
return } else {
shiva(Config.ServerURL, Config.guildID, `${random}`)
}}, 1 * 500)})
...
async function shiva(vanity, token) {
let random = Config.Guards[Math.floor(Math.random() * Config.Guards.length)];
